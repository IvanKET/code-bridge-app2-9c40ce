...
import seederData from "./kelulusan.seeder.json";
...
const KelulusanSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("kelulusan").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(KelulusanSeederDialogComponent);
