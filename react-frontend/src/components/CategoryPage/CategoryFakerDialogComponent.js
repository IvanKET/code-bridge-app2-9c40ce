...
import categoryFakerFactory from "./categoryFakerFactory";
...
const CategoryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = categoryFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("category").create(elem));
...
export default connect(mapState, mapDispatch)(CategoryFakerDialogComponent);
